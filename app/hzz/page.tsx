...
// --- helper: group sections by top-level number (1,2,3…)
...
// --- helper: media query (narrow = < 1024px)
function useIsNarrow(minWidthPx = 1024) {
  const [narrow, setNarrow] = React.useState<boolean>(() =>
    typeof window === "undefined" ? false : window.innerWidth < minWidthPx,
  );
  React.useEffect(() => {
    function onResize() {
      setNarrow(window.innerWidth < minWidthPx);
    }
...
export default function HzzPage() {
  ...
  // global flag: do we have any right-side data in any section?
  const hasAnyRightData = React.useMemo(() => {
    return UI_SECTIONS.some((s) => {
      const v = data[s.id]?.right ?? {};
  ...
  React.useEffect(() => {
    if (isNarrow && hasAnyRightData) setViewMode("right");
  }, [isNarrow, hasAnyRightData]);
  ...
  async function generateFromExample() {
    ...
    try {
      ...
      if (isNarrow) setViewMode("right"); // ⇐ prebacivanje na desni panel u uskom
    } finally {
  ...
  async function debugWebhook() {
    ...
    try {
      ...
      for (;;) {
        ...
        if (js?.status === "done") {
          const exMap = js?.result?.examples ?? js?.examples ?? null;
  ...
  return (
    <div className="h-screen overflow-hidden bg-neutral-50 text-neutral-900 dark:bg-neutral-900 dark:text-neutral-100">
      <div className="flex h-full p-3 min-h-0">
        {/* --- NARROW: jedan panel, ovisno o viewMode --- */}
        {isNarrow ? (
          viewMode === "left" ? (
            // SAMO LIJEVI PANEL (full width)
            <div className="flex flex-col bg-white border rounded-md min-h-0 overflow-hidden flex-1 dark:bg-neutral-900 dark:border-neutral-700">
              <div className="sticky top-0 z-30 bg-white border-b dark:bg-neutral-900 dark:border-neutral-700">
                <div className="p-3 flex items-center justify-between gap-3">
                  <div>
                    <h2 className="text-base font-semibold">Pitanja i ulazni podaci</h2>
                  ...
                  <div className="flex items-center gap-2">
                    {hasAnyRightData && (
                      <button
                        ...
                        className="px-3 py-1.5 rounded border text-sm border-neutral-300 dark:border-neutral-700
              ...
              <div className="flex-1 overflow-y-auto min-h-0 p-3 space-y-3">
                <label className="text-sm font-medium">Kratak opis tvog poslovnog plana (idea)</label>
                <textarea
                ...
                <div aria-busy={isGenerating}>
                  {isGenerating ? (
                    <>
                      Generiranje primjera može trajati do 3 minute...
                      <br />
                      Molimo za strpljenje (slobodno se vrati kasnije provjeriti rezultate).
                    </>
                  ) : (
                    ""
                  )}
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-baseline gap-3">
                    <button
                      ...
                      className="text-sm px-2 py-1 border rounded dark:border-neutral-700"
                      aria-label="Natrag"
                      title="Natrag na pitanja"
                    >
                      ← Natrag
                    </button>
                    <h2 className="text-lg font-semibold">Poslovni plan</h2>
                    <span className="text-sm text-neutral-600 dark:text-neutral-400">
                      ({active}) {getSection(active).title}
                    </span>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto min-h-0 p-3">
                {rightIsBlank ? (
                  <div className="p-4 text-sm text-neutral-500 dark:text-neutral-400">
                    Još nema podataka za ovu sekciju. Koristi <b>“Generiraj primjer”</b> na lijevoj strani.
                  </div>
                ) : (
                  <SectionForm
          ...
          <>
              ...
              <div className="sticky top-0 z-30 bg-white border-b dark:bg-neutral-900 dark:border-neutral-700">
                <div className="p-3 flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    {hasAnyRightData && (
                      <button
                        onClick={() => setActive(UI_SECTIONS[0]?.id ?? "1")}
                        className="px-3 py-1.5 rounded border text-sm border-neutral-300 dark:border-neutral-700
                                   bg-white hover:bg-neutral-100 dark:bg-neutral-900 dark:hover:bg-neutral-800"
                        title="Idi na plan"
                      >
            ...
            {/* DESNO */}
            <div
              className="flex flex-col bg-white border rounded-md min-h-0 overflow-hidden flex-1 dark:bg-neutral-900 dark:border-neutral-700"
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto min-h-0 p-3">
                {Object.values(data[active].right).every((v) => !v) ? (
                  <div className="p-4 text-sm text-neutral-500 dark:text-neutral-400">
